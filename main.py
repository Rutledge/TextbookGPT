...
async def upsert_file(
  ...
  try:
    metadata_obj = (DocumentMetadata.parse_raw(metadata)
                    if metadata else DocumentMetadata())
  except:
...
async def query_main(request: QueryRequest = Body(...), ):
  """
  Query the deep learning textbook. Search through the content of the book 'Deep Learning, Goodfellow et Al. 2016'.
  """
  try:
...
@sub_app.post(
  ...
  description=
  "Accepts search query objects to array each with query and optional filter. Break down complex questions into sub-questions. Refine results by criteria, e.g. time / source, don't do this often. Split queries if ResponseTooLargeError occurs.",
)
